...
const TemperatureChart = () => {
    const [data, setData] = useState(
      monthOrder.map((month) => ({
        name: month,
        value: Math.floor(Math.random() * (50 - 20 + 1)) + 20, // Initial random values for each month (20-50)
      }))
    );
      ...
      const interval = setInterval(() => {
        setData((prevData) => {
          ...
          const updatedData = prevData.map((entry) => ({
            ...entry,
            value: Math.floor(Math.random() * (50 - 20 + 1)) + 20, // New random value for each month (20-50)
          }));
